...
function PhotoSelect({ photos, onComplete, onBack }) {
  ...
  return (
    ...
    <div className="photoselect-container">
                  ...
                  <div className="photoselect-frame-slot" key={index}>
                    {selectedPhotos[index] ? (
                      ...
                      <div className="empty-slot">
                        <span className="photoselect-photo-placeholder">{`사진 ${index + 1}`}</span>
                      </div>
                    )}
                  </div>
                ))}
        ...
        <div className="photoselect-photo-grid">
          {photos.map((photoUrl, index) => {
            ...
            return (
              <div
                key={index}
                className={`photoselect-photo-item ${isSelected ? 'selected' : ''}`}
                onClick={() => togglePhotoSelection(photoUrl)}
              >
                <img src={photoUrl} alt={`촬영된 사진 ${index + 1}`} />
                {isSelected && (
      ...
      <div 
        className={`next-button-container ${selectedPhotos.length !== 4 ? 'disabled' : ''}`}
        onClick={handleNext}
      >
        <div className={`next-button ${selectedPhotos.length !== 4 ? 'disabled' : ''}`}>
